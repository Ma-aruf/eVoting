                    ...
                    {results.positions.length > 0 && (
                        <div className="flex items-center justify-between bg-white border rounded-lg p-4">
                            <button
                                ...
                                className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition"
                            >
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-lg font-semibold text-gray-800">
                                    {currentPositionIndex + 1} of {results.positions.length}
                                </p>
                            </div>
                        ...
                        <section className="bg-white border rounded-lg overflow-hidden">
                            <div className="bg-gray-50 px-6 py-4 border-b">
                                <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                                    <div>
                                        <h3 className="text-lg font-semibold text-gray-800">{currentPosition.position_name}</h3>
                                        <p className="text-sm text-gray-500">
                                            Total Votes: {currentPosition.total_votes} •
                                            Skipped: {currentPosition.skipped_votes} ({currentPosition.skipped_percentage.toFixed(1)}%) •
                                            Valid Votes: {currentPosition.total_votes - currentPosition.skipped_votes}
                                        </p>
                                    </div>

                                    <div className="mt-2 md:mt-0">
                                        <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                                            Position #{currentPosition.display_order}
                                        </span>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200">
                                        ...
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            {currentPosition.candidates.map((candidate, index) => {
                                                ...
                                                return (
                                                    <tr key={candidate.id} className={isWinner ? 'bg-green-50' : ''}>
                                            ...
                                            {/* Skipped Votes Row */}
                                            {currentPosition.skipped_votes > 0 && (
                                                <tr className="bg-yellow-50">
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                        <div className="text-sm font-semibold text-yellow-700">
                                                            {currentPosition.skipped_votes.toLocaleString()}
                                                        </div>
                                                    </td>
                                                    {showPercentages && (
                                                        <td className="px-6 py-4 whitespace-nowrap">
                                                            <div className="text-sm text-yellow-700">
                                                                {currentPosition.skipped_percentage.toFixed(2)}%
                                                            </div>
                                                        </td>
                                                    )}
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                        <div className="flex items-center">
                                                            <div className="w-full bg-gray-200 rounded-full h-2.5">
                                                                <div
                                                                    ...
                                                                    style={{ width: `${currentPosition.skipped_percentage}%` }}
                                                                ></div>
                                                            </div>
                                                            <span className="ml-3 text-xs text-yellow-600">
                                                                {currentPosition.skipped_percentage.toFixed(0)}%
                                                            </span>
                                                        </div>
                                                    </td>
                                                </tr>
                                ...
                                <div className="p-6">
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        {currentPosition.candidates.map((candidate, index) => {
                                            ...
                                            const barWidth = maxVotes > 0 ? (candidate.vote_count / maxVotes) * 100 : 0;
                                            const isWinner = index === 0 && candidate.vote_count > 0;
                                            const percentageOfTotal = currentPosition.total_votes > 0
                                                ? (candidate.vote_count / currentPosition.total_votes) * 100
                                                : 0;
                                            ...
                                            <div className="border border-yellow-200 rounded-lg p-4 bg-yellow-50">
                                                ...
                                                <div className="mt-4 space-y-2">
                                                    <div className="flex justify-between text-sm">
                                                        ...
                                                        <span className="font-semibold text-yellow-700">{currentPosition.skipped_votes}</span>
                                                    </div>

                                                    {showPercentages && (
                                                        <div className="flex justify-between text-sm">
                                                            <span className="text-gray-600">Percentage:</span>
                                                            <span className="font-semibold text-yellow-700">{currentPosition.skipped_percentage.toFixed(1)}%</span>
                                                        </div>
                                                    )}
