...
export default function ImageUpload({onUploadSuccess, currentImageUrl, className = '', disabled = false}: ImageUploadProps) {
    ...
    return <div className={className}>
        <input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/webp" onChange={handleFileSelect} className="hidden" aria-label="Choose candidate photo" disabled={disabled}/>
        {preview ? <div className="relative group">
            ...
            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center gap-2">
                <IconButton label="Change photo" icon={<FiImage aria-hidden="true"/>} size="compact" onClick={() => fileInputRef.current?.click()} disabled={uploading || disabled} className="bg-white"/>
                <IconButton label="Remove photo" icon={<FiTrash2 aria-hidden="true"/>} size="compact" onClick={handleRemove} disabled={uploading || disabled} className="bg-white text-red-600"/>
            </div>
        ...
        </div> : <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading || disabled} className="w-24 h-24 border-2 border-dashed border-gray-300 rounded-lg hover:border-blue-400 hover:bg-blue-50/50 transition flex flex-col items-center justify-center gap-1 disabled:opacity-50">
